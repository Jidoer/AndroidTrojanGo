...
public class ProxyService extends VpnService implements TestConnection.OnResultListener {
    ...
    private final IBinder mBinder = new ITrojanService.Stub() {
        ...
        public void testConnection(String testUrl) {
            if (state != STARTED) {
                onResult(TUN2SOCKS5_SERVER_HOST, false, 0L, "测试前请先连接 [ProxyService not yet connected.]");
                return;
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        trojanPort = ProxyFileHelper.GetProxySocksPort(Globals.getProxyConfigPath());
        LogHelper.i("TrojanCat", "TrojanCat port is " + trojanPort);
        Log.e("TrojanCat", "TrojanCat port is " + trojanPort);

       ///ProxyFileHelper.ChangeListenPort(Globals.getProxyConfigPath(), trojanPort); don't change port!!
       ProxyFileHelper.ShowConfig(Globals.getProxyConfigPath());
        ...
        Trojan.runClient(Globals.getProxyConfigPath());

        long clashSocksPort = 1080; // default value in case fail to get free port
        if (enable_clash) {

            clashSocksPort = trojanPort -1;
        ...
        openMainActivityIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
        PendingIntent pendingOpenMainActivityIntent = PendingIntent.getActivity(this, 0, openMainActivityIntent, 0);
        String RunningStatusStr = "listening on port: " + tun2socksPort;
        final String channelId = getString(R.string.notification_channel_id);
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, channelId)
                .setSmallIcon(R.mipmap.icon)
                .setContentTitle("TrojanCat Active")
                .setContentText(RunningStatusStr)
                .setStyle(new NotificationCompat.BigTextStyle()
                        .bigText(RunningStatusStr))
                .setPriority(NotificationCompat.PRIORITY_HIGH)
                // Set the intent that will fire when the user taps the notification
                .setContentIntent(pendingOpenMainActivityIntent)
                .setAutoCancel(false)
